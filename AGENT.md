...
- `study`
  学习资料目录。后续每新增一个接口，都需要把该接口涉及到的后端知识整理到这里。
- `dist`
  构建产物，不作为日常改动入口。
- `logs`
  运行日志，不作为功能开发入口。
...
- 新增接口时，同时补齐：
  ...
  - 权限装饰器
  - 必要的中文注释
- 新增接口时，必须同步在 `study/` 中新增或更新对应学习文档，至少覆盖：
  - 接口用途
  - 请求链路
  - DTO / 校验知识点
  - 鉴权 / 权限 / 项目作用域知识点
  - Service / 数据层实现要点
  - 异常处理与统一响应要点
- 修改已有接口时，若接口行为、鉴权、参数、数据模型或响应结构发生变化，也必须同步更新 `study/` 中对应文档。
- `study/` 中的文档优先按“接口维度”组织，建议路径形如 `study/interfaces/<模块名>/<接口名>.md`。
...
- 不要新增接口后只改代码不补学习资料；这在当前仓库中视为未完成交付。
...
- 当前 `study/` 目录用于沉淀接口相关后端知识，后续新增接口时要把“代码实现”与“知识整理”视为同一项交付。
- 当前已有 `README.md`，若后续继续补文档，建议优先维护启动方式、环境变量、权限模型与学习资料索引之间的一致性。
